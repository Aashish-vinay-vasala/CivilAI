  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Schedules", value: schedules.length, icon: Calendar, color: "text-blue-400 bg-blue-500/10" },
          { label: "Active",          value: schedules.filter((s) => s.enabled).length, icon: CheckCircle, color: "text-emerald-400 bg-emerald-500/10" },
          { label: "Report Types",    value: 3, icon: Bell, color: "text-amber-400 bg-amber-500/10" },
        ].map((s, i) => (
      ...
      <div className="space-y-3">
        {schedules.map((sc, i) => (
          <motion.div key={sc.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
            className={`bg-card border rounded-2xl p-5 transition-all ${sc.enabled ? "border-border" : "border-border/40 opacity-60"}`}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3 flex-1 min-w-0">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${sc.enabled ? "gradient-blue" : "bg-secondary"}`}>
                  <Mail className={`w-4 h-4 ${sc.enabled ? "text-white" : "text-muted-foreground"}`} />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{TYPE_FREQ[sc.type]} · {sc.project}</p>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button onClick={() => toggleEnabled(sc.id)}
                  className={`relative w-9 h-5 rounded-full transition-colors ${sc.enabled ? "bg-blue-500" : "bg-secondary border border-border"}`}>
                ...
                <button onClick={() => sendNow(sc)} disabled={sending === sc.id}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-secondary text-xs text-foreground hover:bg-secondary/80 transition-colors">
                  {sending === sc.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Send className="w-3 h-3" />}
                  Send Now
                </button>
                <button onClick={() => removeSchedule(sc.id)} className="p-1.5 text-muted-foreground hover:text-red-400 transition-colors">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-semibold text-foreground">New Report Schedule</h3>
                <button onClick={() => setShowCreate(false)} className="text-muted-foreground hover:text-foreground"><X className="w-4 h-4" /></button>
              </div>
              <div className="space-y-3">
                <div><label className="text-xs text-muted-foreground mb-1 block">Schedule Name *</label>
                  <input className={inputClass} placeholder="e.g. Weekly Summary" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
                </div>
                <div><label className="text-xs text-muted-foreground mb-1 block">Type</label>
                  <select className={inputClass} value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value as Schedule["type"] }))}>
                    <option value="daily">Daily</option>
                    <option value="weekly">Weekly</option>
                    <option value="alert">Alert</option>
                  </select>
                </div>
                <div><label className="text-xs text-muted-foreground mb-1 block">Recipient Emails * (comma-separated)</label>
                  <input className={inputClass} placeholder="pm@company.com, eng@company.com" value={form.emails} onChange={(e) => setForm((f) => ({ ...f, emails: e.target.value }))} />
                </div>
                <div><label className="text-xs text-muted-foreground mb-1 block">Project</label>
                  <input className={inputClass} placeholder="All Projects" value={form.project} onChange={(e) => setForm((f) => ({ ...f, project: e.target.value }))} />
                </div>
              </div>
              <div className="flex gap-3 mt-5">
                ...
                <button onClick={handleCreate} className="flex-1 py-2 rounded-xl gradient-blue text-white text-sm flex items-center justify-center gap-2">
