...
const ghostBtn =
  "flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm font-medium text-white/70 hover:text-white whitespace-nowrap transition-all hover:scale-105";
const solidGreenBtn =
...
export default function SchedulingPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "inprogress": case "in_progress": return "bg-cyan-500/10 text-cyan-400 border-cyan-500/20";
      case "delayed": return "bg-red-500/10 text-red-400 border-red-500/20";
      case "atrisk": return "bg-amber-500/10 text-amber-400 border-amber-500/20";
  ...
  const kpis = [
    { label: "Total Tasks", value: totalTasks.toString(), icon: Calendar, accent: "cyan" as const },
    { label: "Overall Progress", value: `${overallProgress}%`, icon: TrendingDown, accent: "amber" as const },
    { label: "Delayed / At Risk", value: overdueTasks.toString(), icon: AlertTriangle, accent: "red" as const },
    { label: "Completed", value: completedTasks.toString(), icon: CheckCircle, accent: "green" as const },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h1 className="text-4xl font-bold text-white tracking-tight">Scheduling</h1>
          <p className="text-white/35 text-[13px] mt-1">AI-powered delay prediction &amp; task management</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <button className={ghostBtn} style={glassButtonStyle} onClick={() => setAddTaskOpen(true)}>
            <Plus className="w-4 h-4 text-emerald-400" />Add Task
          </button>
          <button className={ghostBtn} style={glassButtonStyle} onClick={() => exportScheduleReport(tasks, selectedProject?.name || "Project")}>
            <span>↓</span>Export PDF
          </button>
          <button className={ghostBtn} style={glassButtonStyle} onClick={() => setWhatIfOpen(!whatIfOpen)}>
            <Clock className="w-4 h-4 text-cyan-400" />What-If
          </button>
          <button className={ghostBtn} style={glassButtonStyle} onClick={() => setPredictOpen(!predictOpen)}>
            <AlertTriangle className="w-4 h-4 text-amber-400" />Predict Delays
          </button>
          <button className={ghostBtn} style={glassButtonStyle} onClick={() => setRecoveryOpen(!recoveryOpen)}>
            <Wrench className="w-4 h-4 text-emerald-400" />Recovery Plan
          </button>
      ...
      {selectedProject && (
        <div className="flex items-center gap-3 px-4 py-2 rounded-xl flex-wrap"
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-cyan-400 font-medium">{selectedProject.name}</span>
          </div>
          {tasks.length > 0 && (
          ...
          <span className="text-xs text-white/35 ml-auto">
            Overall Progress: <span className="text-white font-medium">{overallProgress}%</span>
          </span>
        </div>
      )}
      ...
      {/* ML Prediction */}
      {!mlLoading && mlDelay && (() => {
        const riskAccent = mlDelay.risk_level === "High" ? ACCENT.red : mlDelay.risk_level === "Medium" ? ACCENT.amber : ACCENT.green;
        return (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            className="glass-card p-5" style={{ borderColor: riskAccent.border }}>
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                  style={{ background: ACCENT.amber.bg, border: `1px solid ${ACCENT.amber.border}`, boxShadow: `0 0 16px ${ACCENT.amber.shadow}` }}>
                  <Brain className="w-5 h-5 text-amber-400" />
                </div>
                <div>
                  <p className="text-xs text-white/35">AI Delay Prediction</p>
                  <p className="text-xl font-bold text-white">{mlDelay.probability}% probability of delay</p>
        ...
        {addTaskOpen && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setAddTaskOpen(false)} className="p-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/5">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="col-span-2">
                <label className="text-xs text-white/35 mb-1.5 block">Task Name *</label>
                <input placeholder="e.g. Foundation Excavation" value={newTask.task_name}
                  onChange={(e) => setNewTask({ ...newTask, task_name: e.target.value })} className={glassInputClass} style={glassInputStyle} />
              </div>
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Phase</label>
                <input placeholder="e.g. Structure" value={newTask.phase}
                  onChange={(e) => setNewTask({ ...newTask, phase: e.target.value })} className={glassInputClass} style={glassInputStyle} />
              </div>
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Assignee *</label>
                <input placeholder="Worker name" value={newTask.assignee}
                  onChange={(e) => setNewTask({ ...newTask, assignee: e.target.value })} className={glassInputClass} style={glassInputStyle} />
              </div>
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Priority</label>
                <select value={newTask.priority}
                  onChange={(e) => setNewTask({ ...newTask, priority: e.target.value })} className={glassInputClass} style={glassInputStyle}>
              ...
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Budget ($)</label>
              ...
              <div>
                ...
                <select value={newTask.status}
                  onChange={(e) => setNewTask({ ...newTask, status: e.target.value })} className={glassInputClass} style={glassInputStyle}>
              ...
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Actual Progress %</label>
              ...
              <div>
                ...
                <input type="date" value={newTask.planned_start}
                  onChange={(e) => setNewTask({ ...newTask, planned_start: e.target.value })} className={glassInputClass} style={glassInputStyle} />
              </div>
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">End Date</label>
                <input type="date" value={newTask.planned_end}
                  onChange={(e) => setNewTask({ ...newTask, planned_end: e.target.value })} className={glassInputClass} style={glassInputStyle} />
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={handleAddTask} className={solidGreenBtn}>
                <Plus className="w-4 h-4" />Save to Database
              </button>
              <button className={ghostBtn} style={glassButtonStyle} onClick={() => setAddTaskOpen(false)}>Cancel</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {whatIfOpen && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setWhatIfOpen(false)} className="p-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/5">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <textarea placeholder="Describe scenario (e.g. Steel delivery delayed 2 weeks)"
                ...
                rows={3} className={`col-span-2 ${glassInputClass} resize-none`} style={glassInputStyle} />
            ...
            <button onClick={runWhatIf} disabled={whatIfLoading} className={primaryBtn} style={gradientButtonStyle}>
              {whatIfLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Clock className="w-4 h-4" />}
              Run Analysis
            </button>
            {whatIfResult && (
              <div className="mt-4 p-4 rounded-xl" style={{ background: "rgba(0,212,255,0.05)", border: "1px solid rgba(0,212,255,0.15)" }}>
                <MarkdownText text={whatIfResult} className="text-sm text-white/70 leading-relaxed" />
              </div>
            )}
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {predictOpen && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-white">AI Delay Prediction</h3>
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Weather Conditions</label>
                <select value={predictForm.weather_conditions}
                  onChange={(e) => setPredictForm({ ...predictForm, weather_conditions: e.target.value })} className={glassInputClass} style={glassInputStyle}>
                  {["Normal", "Rainy", "Extreme Heat", "Storms", "Winter"].map(w => <option key={w} value={w}>{w}</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Labor Availability</label>
                <select value={predictForm.labor_availability}
                  onChange={(e) => setPredictForm({ ...predictForm, labor_availability: e.target.value })} className={glassInputClass} style={glassInputStyle}>
                  {["Full", "Partial", "Short-staffed"].map(w => <option key={w} value={w}>{w}</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Material Status</label>
                <select value={predictForm.material_status}
                  onChange={(e) => setPredictForm({ ...predictForm, material_status: e.target.value })} className={glassInputClass} style={glassInputStyle}>
                  {["Available", "Delayed", "Backordered"].map(w => <option key={w} value={w}>{w}</option>)}
                </select>
              </div>
            </div>
            <button onClick={runPredictDelays} disabled={predictLoading} className={primaryBtn} style={gradientButtonStyle}>
              {predictLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Brain className="w-4 h-4" />}
              Predict Delays
            </button>
            {predictResult && (
              <div className="mt-4 p-4 rounded-xl" style={{ background: "rgba(0,212,255,0.05)", border: "1px solid rgba(0,212,255,0.15)" }}>
                <MarkdownText text={predictResult} className="text-sm text-white/70 leading-relaxed" />
              </div>
            )}
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {recoveryOpen && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            className="glass-card p-6" style={{ borderColor: ACCENT.green.border }}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-white">AI Recovery Plan Generator</h3>
              <button onClick={() => setRecoveryOpen(false)} className="p-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/5">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Delay (days)</label>
                <input type="number" min="0" value={recoveryForm.delay_days}
                  onChange={(e) => setRecoveryForm({ ...recoveryForm, delay_days: parseInt(e.target.value) || 0 })} className={glassInputClass} style={glassInputStyle} />
              </div>
              <div>
                <label className="text-xs text-white/35 mb-1.5 block">Budget Remaining ($)</label>
                <input type="number" min="0" value={recoveryForm.budget_remaining}
                  onChange={(e) => setRecoveryForm({ ...recoveryForm, budget_remaining: parseFloat(e.target.value) || 0 })} className={glassInputClass} style={glassInputStyle} />
              </div>
              <div className="col-span-2">
                <label className="text-xs text-white/35 mb-1.5 block">Delay Causes (comma-separated)</label>
                <input placeholder="e.g. Steel delivery delay, Permit hold-up" value={recoveryForm.delay_causes}
                  onChange={(e) => setRecoveryForm({ ...recoveryForm, delay_causes: e.target.value })} className={glassInputClass} style={glassInputStyle} />
              </div>
              <div className="col-span-2">
                <label className="text-xs text-white/35 mb-1.5 block">Available Resources (comma-separated)</label>
                <input placeholder="e.g. Extra crew, Overtime budget" value={recoveryForm.available_resources}
                  onChange={(e) => setRecoveryForm({ ...recoveryForm, available_resources: e.target.value })} className={glassInputClass} style={glassInputStyle} />
              </div>
            </div>
            <button onClick={runRecoveryPlan} disabled={recoveryLoading} className={solidGreenBtn}>
              {recoveryLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wrench className="w-4 h-4" />}
              Generate Recovery Plan
            </button>
            {recoveryResult && (
              <div className="mt-4 p-4 rounded-xl" style={{ background: "rgba(0,212,255,0.05)", border: "1px solid rgba(0,212,255,0.15)" }}>
                <MarkdownText text={recoveryResult} className="text-sm text-white/70 leading-relaxed" />
              </div>
            )}
          </motion.div>
        )}
        ...
        {extractedTasks.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              <div>
                <h3 className="font-semibold text-white">Tasks Found in Document</h3>
                <p className="text-xs text-white/35 mt-0.5">Review and add to your task list</p>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={addAllExtractedTasks} disabled={addingExtracted === "all"}
                ...
                <button onClick={() => setExtractedTasks([])} className="text-white/40 hover:text-white">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
                ...
                <div key={idx} className="flex items-start gap-3 p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.03)" }}>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {task.phase && <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.5)" }}>{task.phase}</span>}
                      {task.assignee && <span className="text-xs text-white/35">{task.assignee}</span>}
                      {task.planned_start && <span className="text-xs text-white/35">{task.planned_start} → {task.planned_end || "?"}</span>}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => addExtractedTask(task, idx)}
      ...
      <div className="flex gap-0.5 p-1 rounded-xl w-fit"
        ...
        <button onClick={() => setActiveTab("gantt")}
          className="px-3.5 py-1.5 rounded-lg text-[13px] font-medium whitespace-nowrap transition-colors"
          style={tabBtnStyle(activeTab === "gantt")}>
          📊 Gantt Chart
        </button>
      </div>
      ...
      {activeTab === "tasks" && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative">
                ...
                <input placeholder="Search tasks..." value={search} onChange={(e) => setSearch(e.target.value)}
                  className="pl-8 pr-3 py-1.5 rounded-xl text-xs text-white placeholder:text-white/30 outline-none border focus:border-cyan-500/50 w-36"
                  style={glassInputStyle} />
              </div>
          ...
          {/* Table Header */}
          <div className="grid grid-cols-12 gap-2 px-4 py-2 text-xs text-white/35 font-medium mb-2"
          ...
          {tasksLoading ? (
            <div className="flex items-center justify-center py-8">
              ...
              <p className="ml-2 text-sm text-white/35">Loading tasks...</p>
            </div>
          ) : filteredTasks.length === 0 ? (
            <div className="text-center py-8">
              <Calendar className="w-10 h-10 text-white/30 mx-auto mb-2" />
              <p className="text-sm text-white/35">No tasks found</p>
              ...
              {filteredTasks.map((task, i) => (
                <motion.div key={task.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                  ...
                  <div className="col-span-3">
                    <p className="text-sm font-medium text-white truncate">{task.task_name}</p>
                    <p className="text-xs text-white/35">{task.planned_start} → {task.planned_end}</p>
                    {task.delay_days > 0 && <p className="text-xs text-red-400">{task.delay_days}d delay</p>}
                  </div>
                  <div className="col-span-1">
                    <span className="text-xs px-1.5 py-0.5 rounded-md truncate block" style={{ background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.5)" }}>
                      {task.phase || "—"}
                    </span>
                  </div>
                  <div className="col-span-2">
                    <p className="text-xs text-white truncate">{task.assignee || "—"}</p>
                  </div>
                  <div className="col-span-3">
                    {editingTask === task.id ? (
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-white w-8">{editProgress}%</span>
                        </div>
                        <select value={editStatus} onChange={(e) => setEditStatus(e.target.value)}
                          className="w-full px-2 py-1 rounded-lg text-xs text-white outline-none border" style={glassInputStyle}>
                          <option value="pending">Pending</option>
                          <option value="inprogress">In Progress</option>
                          <option value="delayed">Delayed</option>
                          <option value="atrisk">At Risk</option>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-white w-8 text-right">{task.actual_progress || 0}%</span>
                      </div>
                    )}
                  </div>
                  <div className="col-span-1">
                    ...
                    {editingTask === task.id ? (
                      <>
                        ...
                        <button onClick={() => setEditingTask(null)}
                          className="p-1.5 rounded-lg hover:bg-white/5 transition-colors" style={{ background: "rgba(255,255,255,0.05)" }}>
                      ...
                      <>
                        ...
                        <button onClick={() => { setEditingTask(task.id); setEditProgress(task.actual_progress || 0); setEditStatus(task.status || "pending"); setEditBudget(task.budget || 0); }}
                          className="p-1.5 rounded-lg hover:bg-cyan-500/10 transition-colors opacity-0 group-hover:opacity-100">
      ...
      {activeTab === "gantt" && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-white">Gantt Chart</h3>
            {selectedProject && (
          ...
          {tasksLoading ? (
            ...
            <div className="text-center py-8">
              <Calendar className="w-10 h-10 text-white/30 mx-auto mb-2" />
              <p className="text-sm text-white/35">No tasks to display on Gantt</p>
              <button onClick={() => setAddTaskOpen(true)}
                className="mt-3 flex items-center gap-2 mx-auto px-4 py-2 rounded-xl text-white text-xs font-medium transition-all hover:scale-105"
                style={gradientButtonStyle}>
                Add Tasks
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="font-semibold text-white text-[14px]">S-Curve Progress</h3>
            <p className="text-[11px] text-white/35 mt-0.5">Planned vs Actual cumulative %</p>
          </div>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={sCurveData}>
            ...
            <Area type="monotone" dataKey="planned" stroke="#00D4FF" fill="url(#planned)" strokeWidth={2} name="Planned" />
            <Area type="monotone" dataKey="actual" stroke="#10B981" fill="url(#actual)" strokeWidth={2} name="Actual" />
          </AreaChart>
        </ResponsiveContainer>
        <div className="flex gap-4 mt-2">
          <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-cyan-400" /><span className="text-xs text-white/35">Planned</span></div>
          <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-400" /><span className="text-xs text-white/35">Actual</span></div>
        </div>
      </motion.div>
      ...
      {analysis && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <div className="flex items-center gap-2 mb-3">
            <Calendar className="w-5 h-5 text-cyan-400" />
            <h3 className="font-semibold text-white text-[15px]">AI Schedule Analysis</h3>
          </div>
